...
class InfractionsFragmentTest{

    lateinit var scenario : FragmentScenario<InfractionsFragment>

    @Before
    fun setup()
    {
        val sMobileApp = ApplicationProvider.getApplicationContext<SmobileApp>()
        val fakeComponent = DaggerAppComponent.builder().application(sMobileApp).appModule(FakeAppModule()).build()
