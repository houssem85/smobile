 ...
 */
class DraggableLinearLayout(context: Context, attrs: AttributeSet) : FrameLayout(context, attrs) {
    ...
    private fun draggableChildSetup()
    {
        val childCount= this.childCount
        for(i in 0 until childCount){
            val child = this.getChildAt(i)
            child.setOnTouchListener { _, event ->
                val viewParent = this@DraggableLinearLayout.parent as View
                val parentHeight = viewParent.height
                val parentWidth = viewParent.width
                val xMax = parentWidth - this@DraggableLinearLayout.width
                val xMiddle = parentWidth / 2
                val yMax = parentHeight - this@DraggableLinearLayout.height
                val yMiddle = parentHeight / 2

                when (event.actionMasked) {
                    MotionEvent.ACTION_DOWN -> {
                        widgetDX = this@DraggableLinearLayout.x - event.rawX
                        widgetDY = this@DraggableLinearLayout.y - event.rawY
                        widgetInitialX = this@DraggableLinearLayout.x
                        widgetInitialY = this@DraggableLinearLayout.y
                    }
                    MotionEvent.ACTION_MOVE -> {
                        var newX = event.rawX + widgetDX
                        newX = max(0F, newX)
                        newX = min(xMax.toFloat(), newX)
                        this@DraggableLinearLayout.x = newX

                        var newY = event.rawY + widgetDY
                        newY = max(0F, newY)
                        newY = min(yMax.toFloat(), newY)
                        this@DraggableLinearLayout.y = newY

                        draggableListener?.onViewMove(this@DraggableLinearLayout)
                    }
                    MotionEvent.ACTION_UP -> {
                        when (stickyAxis) {
                            STICKY_AXIS_X -> {
                                if (event.rawX >= xMiddle) {
                                    if (mAnimate)
                                        this@DraggableLinearLayout.animate().x(xMax.toFloat())
                                            .setDuration(Draggable.DURATION_MILLIS)
                                            .setUpdateListener { draggableListener?.onViewMove(this@DraggableLinearLayout) }
                                            .start()
                                    else
                                        this@DraggableLinearLayout.x = xMax.toFloat()
                                } else {
                                    if (mAnimate)
                                        this@DraggableLinearLayout.animate().x(0F).setDuration(Draggable.DURATION_MILLIS)
                                            .setUpdateListener { draggableListener?.onViewMove(this@DraggableLinearLayout) }
                                            .start()
                                    else
                                        this@DraggableLinearLayout.x = 0F
                                }
                            }
                            STICKY_AXIS_Y -> {
                                if (event.rawY >= yMiddle) {
                                    if (mAnimate)
                                        this@DraggableLinearLayout.animate().y(yMax.toFloat())
                                            .setDuration(Draggable.DURATION_MILLIS)
                                            .setUpdateListener { draggableListener?.onViewMove(this@DraggableLinearLayout) }
                                            .start()
                                    else
                                        this@DraggableLinearLayout.y = yMax.toFloat()
                                } else {
                                    if (mAnimate)
                                        this@DraggableLinearLayout.animate().y(0F)
                                            .setDuration(Draggable.DURATION_MILLIS)
                                            .setUpdateListener { draggableListener?.onViewMove(this@DraggableLinearLayout) }
                                            .start()
                                    else {
                                        if (mAnimate)
                                            this@DraggableLinearLayout.animate().y(0F).setDuration(Draggable.DURATION_MILLIS)
                                                .setUpdateListener {
                                                    draggableListener?.onViewMove(this@DraggableLinearLayout)
                                                }
                                                .start()
                                        else
                                            this@DraggableLinearLayout.y = 0F
                                    }
                                }
                            }
                            STICKY_AXIS_XY -> {
                        ...
                        if (abs(this@DraggableLinearLayout.x - widgetInitialX) <= DRAG_TOLERANCE && abs(this@DraggableLinearLayout.y - widgetInitialY) <= DRAG_TOLERANCE) {
                            child.performClick()
                        }
                    }
                    else -> return@setOnTouchListener false
