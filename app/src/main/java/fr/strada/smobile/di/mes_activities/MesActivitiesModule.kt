...
import fr.strada.smobile.ui.activities.hebdomadaire.Utils.getDisplayedYears
...
class MesActivitiesModule {
    ...
    fun provideDialogChangeYear(context: Context,dialogChangeYearListner: DialogChangeYearListner?):Dialog
    {
        ...
        yearPicker.displayedValues = getDisplayedYears(yearPicker.minValue,yearPicker.maxValue )
        dialogBuilder.setView(dialogView)
        val dialog= dialogBuilder.create()
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE)
        dialog.window!!.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
